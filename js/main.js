...
(function ($) {
	...
	$(window).on("load", function () {
		...
		if (tjPreloader?.length) {
			// only show full loader on first visit, remember decision in localStorage
			if (!localStorage.getItem("preloaderShown")) {
				localStorage.setItem("preloaderShown", "true");
				// run the existing animation sequence
				setTimeout(function () {
				...
				// skip animation, hide immediately and initialize controllers
				tjPreloader.removeClass("is-loading").addClass("is-loaded").hide();
				wowController();
				gsapController();
			}
		} else {
